...
if(isset($_GET['id']) && $_GET['id'] > 0){
    ...
    if($qry->num_rows > 0){
		...
		echo '<script>alert("statusprogres ID is not valid."); location.replace("./?page=statusprogres")</script>';
	}
}else{
	echo '<script>alert("statusprogres ID is Required."); location.replace("./?page=statusprogres")</script>';
}
?>
<style>
